namespace Game.Packets {
    class RoomUpdate : Core.Networking.OutPacket {
        public RoomUpdate(uint id) : base((ushort)Game.Enums.Packets.RoomListUpdate) {
            Append2(id);
            Append2((byte)Enums.RoomUpdateType.Delete);
        }

        public RoomUpdate(Entities.Room r, bool isUpdate)
            : base((ushort)Game.Enums.Packets.RoomListUpdate) {
            Append2(r.ID);
            Append2((byte)(isUpdate ? Enums.RoomUpdateType.Update:Enums.RoomUpdateType.Create));
            addRoomInfo(r);
        }
