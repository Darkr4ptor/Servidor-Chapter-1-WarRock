...
namespace Game.Packets {
    class RoomList : Core.Networking.OutPacket {
        public RoomList()
            : base((ushort)Game.Enums.Packets.RoomList) {
                Fill(3, 0);
        }

        public RoomList(byte page, ArrayList arrList)
            : base((ushort)Game.Enums.Packets.RoomList) {
                Append2(arrList.Count);
                Append2(page);
                Append2(0);
                foreach (Entities.Room r in arrList) {
                    addRoomInfo(r);
                }
